...
from src.email_backend.schemes.dto import PublicMessageCreate, MessageInfo
...
def get_message(box_id: int, current_user_name: str = Depends(get_current_user_name)):
    ...
    with get_db_session() as session:
        if current_user_name:
            ...
            return [msg.model_dump(mode="json") for msg in message_service.get_mail_by_box_id(box_id)]
        else:
            raise HTTPException(
...
def delete_message(message_id: int, box_id: int, current_user_name: str = Depends(get_current_user_name)):
    ...
    with get_db_session() as session:
        if current_user_name:
            message_service = MessageService(session=session)
            return message_service.delete_mail(message_id, box_id).model_dump(mode="json")
        else:
            raise HTTPException(
                status_code=status.HTTP_403_UNAUTHORIZED,
                detail="请登录再操作！"
...
def create_message(message_data: MessageInfo, current_user_name: str = Depends(get_current_user_name)):
    ...
    with get_db_session() as session:
        if current_user_name:
            message_service = MessageService(session=session)
            return message_service.create_mail(message_data).model_dump(mode="json")
        else:
            raise HTTPException(
                status_code=status.HTTP_403_UNAUTHORIZED,
                detail="请登录再操作！"
